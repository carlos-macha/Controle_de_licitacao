...
export class LicitacaoService extends BaseService<Licitacao> {
    constructor(
        ...
        super(licitacaoDAO, "Licitação");
    }

    async insert(licitacao: Omit<Licitacao, "ID">) {
        const existente = await this.dao.find({
            where: {
                CODIGO_LICITACAO: licitacao.CODIGO_LICITACAO,
            },
    ...
    async update(id: number, licitacao: Partial<Licitacao>) {
        if (licitacao.CODIGO_LICITACAO) {
            ...
            if (existente.data.length > 0 && existente.data[0].ID !== id) {
                throw new HttpError(
                    409,
                    "Já existe uma licitação com esse código."
                );
        ...
        return super.update(id, licitacao);
    }

    async delete(id: number) {
        const possuiProdutos = await this.licitacaoProdutoDAO.find({
            where: {
                CODIGO_LICITACAO: id,
            },
        });
        ...
        if (possuiProdutos.data.length > 0) {
            throw new HttpError(409, "A licitação possui produtos vinculados.");
        }

        const possuiResultados = await this.resultadoLicitacaoDAO.find({
            where: {
                CODIGO_LICITACAO: id,
            },
        });
