...
export class UsuarioService extends BaseService<Usuario> {
    constructor(
        ...
        super(usuarioDAO, "Usuário");
    }

    async findSafe(options?: any) {
        const result = await this.dao.find(options);
    ...
    async create(
        usuario: Omit<Usuario, "ID"> & UsuarioInsert
    ...
    async updateSafe(id: number, usuario: UsuarioUpdate) {
        ...
        const usuarioAtualizado = await super.update(id, updateData);
    ...
    async atualizarSenha(id: number, senhaAtual: string, novaSenha: string) {
        const usuario = await this.usuarioDAO.findById(id);
